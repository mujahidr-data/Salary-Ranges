...
function syncEmployeesMappedSheet_() {
  ...
  // Validate Start Date column detection
  if (iStart < 0) {
    Logger.log(`⚠️ Start Date column NOT detected in Base Data! Headers: ${baseHead.slice(0, 10).join(', ')}`);
  }
  ...
  if (rows.length) {
    ...
    // Format Start Date column (O - column 15) as date
    empSh.getRange(2,15,rows.length,1).setNumberFormat('yyyy-mm-dd');
  ...
  // Validate start dates (only log if there are issues)
  const missingStartDateRows = rows.filter(row => !row[14]); // Column O (index 14)
  if (missingStartDateRows.length > 0) {
    Logger.log(`⚠️ ${missingStartDateRows.length} employees missing start dates:`);
    missingStartDateRows.slice(0, 10).forEach((row, idx) => {
...
function reviewRangeProgression() {
  ...
  // ================================
  // CROSS-TRACK VALIDATION: IC > Mgr at equivalent levels
  // ================================
  ...
  // Build lookup map by Region + Aon Code (without track) for cross-track comparison
  const crossTrackMap = new Map();
  
  for (const [groupKey, rows] of groups.entries()) {
    const [region, aonCode, track] = groupKey.split('|');
    const baseKey = `${region}|${aonCode}`;
    
    if (!crossTrackMap.has(baseKey)) {
      crossTrackMap.set(baseKey, { IC: new Map(), Mgr: new Map() });
    }
    
    // Store each level's data by level number (e.g., "L4", "L5", "L5.5", "L6", "L6.5", "L7")
    for (const row of rows) {
      const levelNum = row.level.replace(' IC', '').replace(' Mgr', '');  // Extract "L4", "L5", etc.
      crossTrackMap.get(baseKey)[track].set(levelNum, row);
    }
  }
  
  // Levels where IC should be > Mgr (L4 through L7)
  const crossTrackLevels = ['L4', 'L5', 'L5.5', 'L6', 'L6.5', 'L7'];
  
  for (const [baseKey, tracks] of crossTrackMap.entries()) {
    const [region, aonCode] = baseKey.split('|');
    
    for (const levelNum of crossTrackLevels) {
      const icData = tracks.IC.get(levelNum);
      const mgrData = tracks.Mgr.get(levelNum);
      ...
      if (icData.rangeStart > 0 && mgrData.rangeStart > 0 && icData.rangeStart <= mgrData.rangeStart) {
        issues.push({
          ...
          issue: `IC vs Mgr: ${levelNum} IC (${formatNumber(icData.rangeStart)}) ≤ ${levelNum} Mgr (${formatNumber(mgrData.rangeStart)})`,
          recommended: Math.ceil(mgrData.rangeStart * 1.10),  // Suggest 10% above Mgr
          status: 'Pending'
        });
      }
      ...
      if (icData.rangeMid > 0 && mgrData.rangeMid > 0 && icData.rangeMid <= mgrData.rangeMid) {
        issues.push({
          ...
          issue: `IC vs Mgr: ${levelNum} IC (${formatNumber(icData.rangeMid)}) ≤ ${levelNum} Mgr (${formatNumber(mgrData.rangeMid)})`,
          recommended: Math.ceil(mgrData.rangeMid * 1.10),  // Suggest 10% above Mgr
          status: 'Pending'
        });
      }
      ...
      if (icData.rangeEnd > 0 && mgrData.rangeEnd > 0 && icData.rangeEnd <= mgrData.rangeEnd) {
        issues.push({
          ...
          issue: `IC vs Mgr: ${levelNum} IC (${formatNumber(icData.rangeEnd)}) ≤ ${levelNum} Mgr (${formatNumber(mgrData.rangeEnd)})`,
          recommended: Math.ceil(mgrData.rangeEnd * 1.10),  // Suggest 10% above Mgr
          status: 'Pending'
        });
      }
    }
